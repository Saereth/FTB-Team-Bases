...
FTB Team Bases is a replacement in MC 1.20.1 for earlier base management mods, including FTB Team Dimensions, FTB Team Islands and FTB Dungeons. It includes functionality from all of those:
...
* `dimension_id` - Optional custom dimension ID for the base.
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `chunk_generator` | enum | "multi_biome_void" | Chunk generator type (simple_void/multi_biome_void/custom) |
| `single_biome_id` | string | "" | Force single biome (e.g., "minecraft:the_void") |
| `copy_generator_from_dimension` | string | "" | Copy generator settings from existing dimension |
| `custom_biome_param_list` | string | "minecraft:overworld" | Biome preset for custom generator (overworld/nether) |
